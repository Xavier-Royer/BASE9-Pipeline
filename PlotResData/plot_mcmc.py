...
def createPlots(clusterName, version = ""):

    rootdir = clusterName +"/"
    plotfile = rootdir + clusterName + version + '.png'
    histfile = rootdir + clusterName + version+ '_hist.png'
    # load the data into arrays
    data    = np.loadtxt(rootdir + clusterName + version + ".res" ,skiprows=1,usecols=[0,1,2,3,5])
    age_all = data[:,0]
    feh_all = data[:,1]
    plx_all = data[:,2]
    av_all  = data[:,3]
    ...
    plt.show()
    fig.savefig(histfile,format = "png")
    plt.close('all')
 

 #directory should be the name of the cluster 
def createAllPlots(directory):
    files = os.listdir(directory)
    for file in files:
        if file.removesuffix(".res") != file:
             #file is a .res file
            version = file.removesuffix(".res").removeprefix(directory)
            print(f"Plots for version: {version}")
            createPlots(directory, version= version)
